...
function runPublicExportSmoke(tempRoot) {
  ...
  run('node', ['--input-type=module', '-e', 'import { withPtkScan, bootstrapPtkPage } from "pentestkit/playwright"; if (typeof withPtkScan !== "function") throw new Error("missing withPtkScan"); if (typeof bootstrapPtkPage !== "function") throw new Error("missing bootstrapPtkPage");'], { cwd: tempRoot });
  run('node', ['--input-type=module', '-e', 'import { withPtkScan } from "pentestkit/selenium"; if (typeof withPtkScan !== "function") throw new Error("missing selenium withPtkScan");'], { cwd: tempRoot });
  run('node', ['--input-type=module', '-e', 'import { withPtkScan } from "pentestkit/puppeteer"; if (typeof withPtkScan !== "function") throw new Error("missing puppeteer withPtkScan");'], { cwd: tempRoot });
  run('node', ['-e', 'for (const name of ["puppeteer", "puppeteer-core"]) { try { require.resolve(name); throw new Error(`${name} was installed implicitly`); } catch (error) { if (!String(error.message).includes("Cannot find module")) throw error; } } const { resolvePuppeteer } = require("pentestkit/puppeteer"); try { resolvePuppeteer(); throw new Error("missing Puppeteer did not fail"); } catch (error) { if (!/Install puppeteer or puppeteer-core/.test(error.message)) throw error; }'], { cwd: tempRoot });
