...
class PlaywrightPageAdapter:
    ...
    def evaluate(self, function_source: str, arg=None):
        if arg is None:
            return self.page.evaluate(function_source)
        return self.page.evaluate(function_source, arg)

    def wait_for_timeout(self, ms: int):
        return self.page.wait_for_timeout(ms)


def create_ptk_bridge(page, default_timeout: float = 30, activate: bool = True):
    return PTKBridge(PlaywrightPageAdapter(page), default_timeout=default_timeout, activate=activate)


def wait_for_ptk(page, timeout: float = 30, **options):
...
def with_ptk_scan(
    ...
    **options,
    ...
    bridge = options.pop("bridge", None) or create_ptk_bridge(page, default_timeout=wait_timeout)
