...
class PTKDriver:
    ...
    def _discover_extension_origin(self, timeout: int = 10) -> Optional[str]:
        ...
        while time.time() < deadline:
            ...
            for target in targets:
                candidate = str(target.get("url") or "")
                parsed = urlparse(candidate)
                if parsed.scheme != "chrome-extension" or not parsed.netloc:
                    continue
                if parsed.path not in ("/app.js", "/app_automation.js"):
                    continue
                self._extension_origin = f"{parsed.scheme}://{parsed.netloc}"
                return self._extension_origin
            time.sleep(0.1)
        return None
    ...
    def arm_iast_for_navigation(
        ...
        engine_configs: dict = None,
        timeout: int = 10,
        ttl_seconds: int = 60,
    ) -> dict:
        """Install scoped IAST document-start hooks before navigating this tab."""
        requested_engines = [str(value).strip().upper() for value in (engines or [])]
        if requested_engines and "IAST" not in requested_engines:
        ...
        origin = self._discover_extension_origin(timeout=timeout)
        if not origin:
            return {"ok": False, "applicable": False, "error": "ptk_extension_origin_not_found"}
        control_url = f"{origin}/ptk/automation/control.html"
        try:
            firefox_frame_transport = origin.startswith("moz-extension://")
            if firefox_frame_transport:
                self.driver.get("about:blank")
                control_frame = self.driver.execute_script(
                    """
                ...
                self.driver.switch_to.frame(control_frame)
            else:
                self.driver.get(control_url)
            ...
            result = self.driver.execute_async_script(
                ...
                const run = () => {
                    ...
                    if (control && typeof control.armIastForNavigation === 'function') {
                        Promise.resolve(control.armIastForNavigation(options))
                            .then(callback)
                    ...
                    if (Date.now() >= deadline) {
                        callback({ ok: false, error: 'ptk_automation_control_unavailable' });
                        return;
                    }
                    setTimeout(run, 25);
                };
                ...
                """,
                {
                    ...
                    "scanOptions": {
                        ...
                        "engineConfigs": engine_configs or {},
                    },
        ...
        if origin.startswith("moz-extension://"):
            result.setdefault("transport", "firefox_extension_frame")
        return result
